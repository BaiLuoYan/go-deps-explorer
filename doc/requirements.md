...
| ID | 功能描述 | 优先级 |
|----|---------|--------|
| F5 | 在 go.mod 中有 replace 指令的依赖包，在依赖树中使用特殊图标标识 | P1 |
| F5.1 | 使用 VSCode 内置的 find-replace 或 arrow-swap 图标区分 replace 依赖 | P1 |
| F5.2 | 在依赖包名称旁显示 "→ replaced" 描述 | P1 |
| F5.3 | Hover tooltip 显示详细的替换信息（原路径 → 替换路径） | P1 |
...
| ID | 功能描述 | 优先级 |
|----|---------|--------|
| F7 | 在依赖包源码中能够 Cmd+Click 跳转到其他依赖包或标准库 | P0 |
| F7.1 | 依赖源码文件使用原生 `file://` URI 方式打开，而非自定义 scheme | P0 |
...
## 13. v0.2.1~v0.2.3 版本图标功能需求

### v0.2.1 - 插件图标

| ID | 功能描述 | 优先级 |
|----|---------|--------|
| V21-001 | package.json 新增 `"icon": "icon.png"` 字段，提供扩展图标 | P0 |
| V21-002 | 创建 icon.png 文件：Go 蓝色圆形背景 + 白色依赖树图案，128x128px | P0 |
| V21-003 | package.json view 新增 `"icon": "tree-icon.svg"` 字段，提供侧边栏自定义图标 | P0 |
| V21-004 | 创建 tree-icon.svg 文件：替代默认文件图标的自定义树形图标 | P0 |

### v0.2.2 - 图标微调

| ID | 功能描述 | 优先级 |
|----|---------|--------|
| V22-001 | 调整 tree-icon.svg 树形竖线上下延长，提升视觉平衡性 | P1 |
| V22-002 | 优化图标在不同尺寸下的显示效果 | P1 |

### v0.2.3 - 侧边栏提示修复

| ID | 功能描述 | 优先级 |
|----|---------|--------|
| V23-001 | package.json view 新增 `"contextualTitle": "Go Deps Explorer"` 字段 | P0 |
| V23-002 | 修复拖拽到侧边栏后悬浮显示"资源管理器"而非插件名称的问题 | P0 |
| V23-003 | 确保用户界面显示插件的正确标识和名称 | P0 |
