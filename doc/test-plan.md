...
### 2.11 新增功能测试用例 (v0.2.5 Cmd+Click 只读修复)

| 测试编号 | 测试用例描述 | 输入数据 | 预期结果 |
|---------|-------------|----------|----------|
| **RO-007** | **Cmd+Click 跳转文件只读验证** | **通过 Cmd+Click 跳转到依赖包源码文件** | **文件自动标记为会话级只读，无法编辑但保持完整的 gopls 功能** |
| **RO-008** | **EditorTracker 只读触发时机** | **依赖树点击打开 vs Cmd+Click 跳转打开同一文件** | **两种方式打开的文件都被标记为只读，行为完全一致** |
| **RO-009** | **只读设置在 reveal 之前执行** | **Cmd+Click 跳转到深层依赖文件，观察只读状态和依赖树定位的顺序** | **文件先被标记只读，然后依赖树展开定位到该文件，用户看到的文件已是只读状态** |
| **RO-010** | **只读命令异常处理** | **在不支持只读命令的环境中 Cmd+Click 跳转依赖文件** | **只读命令失败但被捕获，错误信息记录到 Output Channel，不影响跳转和定位功能** |
| **RO-011** | **多次跳转的只读一致性** | **连续 Cmd+Click 跳转多个不同的依赖文件** | **每个文件都被正确标记为只读，不会出现遗漏或重复设置的情况** |
