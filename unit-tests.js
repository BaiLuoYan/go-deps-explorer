...
function extractModuleFromPath(filePath, gopath) {
...
function parseGoModText(goModContent) {
    const dependencies = [];
    // Match single line require
    const singleRequireRegex = /^\s*require\s+([^\s]+)\s+([^\s]+)(?:\s*\/\/\s*indirect)?.*$/gm;
    let match;
    while ((match = singleRequireRegex.exec(goModContent)) !== null) {
        dependencies.push({
            path: match[1],
            version: match[2],
            indirect: match[0].includes('// indirect')
        });
    }
    // Match require block
    const requireBlockRegex = /require\s*\(\s*([\s\S]*?)\s*\)/g;
    const blockMatch = requireBlockRegex.exec(goModContent);
    if (blockMatch) {
        const blockContent = blockMatch[1];
        const lineRegex = /^\s*([^\s]+)\s+([^\s]+)(?:\s*\/\/\s*indirect)?.*$/gm;
        let lineMatch;
        while ((lineMatch = lineRegex.exec(blockContent)) !== null) {
            dependencies.push({
                path: lineMatch[1],
                version: lineMatch[2],
                indirect: lineMatch[0].includes('// indirect')
            });
        }
    }
    return dependencies;
